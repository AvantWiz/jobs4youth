...
-- Build 12 — Guided Opportunity Session Activation
-- Run after jobs4youth_full_product_foundation.sql.
-- Adds durable application-session storage used by the upgraded opportunity flow.
...
create index if not exists idx_opportunity_application_drafts_applicant on public.opportunity_application_drafts(applicant_id, updated_at desc);
create index if not exists idx_opportunity_application_drafts_opportunity on public.opportunity_application_drafts(opportunity_id);
create index if not exists idx_application_submission_payloads_applicant on public.application_submission_payloads(applicant_id, submitted_at desc);
create index if not exists idx_application_submission_payloads_opportunity on public.application_submission_payloads(opportunity_id);
