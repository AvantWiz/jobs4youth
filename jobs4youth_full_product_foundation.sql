-- Jobs4Youth Full Product Foundation SQL
-- Run AFTER your base schema, policies, and the current step6 / step7+step8 SQL.
-- This file adds the foundation for Save/Shortlist + guided application architecture.
...
do $$ begin
  ...
  drop policy if exists "Anyone can view opportunity questions for visible opps" on public.opportunity_screening_questions;
  drop policy if exists "Poster or admin can manage opportunity questions" on public.opportunity_screening_questions;
  drop policy if exists "Anyone can view course questions for visible courses" on public.course_application_questions;
...
create policy "Anyone can view opportunity questions for visible opps" on public.opportunity_screening_questions
for select using (
  exists (
...
for all using (
  exists (
    select 1 from public.opportunities o
    where o.id = opportunity_id
      and (o.posted_by = auth.uid() or exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'))
  )
)
with check (
  exists (
...
create policy "Anyone can view course questions for visible courses" on public.course_application_questions
for select using (
  exists (
...
for all using (
  exists (
    select 1 from public.courses c
    where c.id = course_id
      and (c.posted_by = auth.uid() or exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'))
  )
)
with check (
  exists (
...
for select using (
  exists (
    select 1 from public.courses c where c.id = course_id and c.posted_by = auth.uid()
  )
  or exists (
